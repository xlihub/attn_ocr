...
class TextBox(Box):
    ...
    def _get_np_index(self, index, index_end=None):
        ...
        if index_end:
            ss = "卍{0,}[" + "]{1,}卍{0,}[".join(list(self.text[:index_end])) + "]{1,}"
    ...
    def decode_score(self, mask=np.array([]), original=False):
        def labels_to_text(labels, join=""):
            ...
            for c in labels:
                l = len(alphabet)
                if c == 0 or c == len(alphabet):  # CTC Blank
                    ret.append(join)
                else:
                    ch = alphabet[c]
                    # print(alphabet[c - 1])
    ...
    def get_siamese_rate(self, codes, boolean_anchor, siamese_threshold=0.9, legal_length=6):
        if boolean_anchor:
            ...
            if command == "type":
                if char == 'num':
                    ...
                    if loc == '':
                        ...
                        text_list = re.findall(r"\d+?\d*", text)
                        text = text_list[0] + '年' + text_list[1] + '月'
                    elif loc == '/':
                        if len(text) == 9:

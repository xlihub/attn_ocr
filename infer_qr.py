    ...
    for i in range(batch_loop_cnt):
        ...
        if 'boxes' in results and len(results['boxes']) > 0:
            for index, dt in enumerate(results['boxes']):
                ...
                if len(QR_info) == 0:
                    resize = preprocess2decode(roi, 'resize')
                    im, rects_list, polygon_points_list, QR_info = decodeDisplay(resize)
                    if len(QR_info) == 0:
                        cv2.imwrite(
                            '/home/xli/PaddleDetection/output/inference_results/resize_' + str(index) + '.jpg',
                            resize)
                        pil_img = Image.open(
                            '/home/xli/PaddleDetection/output/inference_results/resize_' + str(index) + '.jpg')
                        ...
                        contrast.save(
                            '/home/xli/PaddleDetection/output/inference_results/contrast_' + str(index) + '.jpg')
                        cont = cv2.imread(
                            '/home/xli/PaddleDetection/output/inference_results/contrast_' + str(index) + '.jpg')
